...
3.  **Configure as variáveis de ambiente:**
    ...
    Se estiver usando Docker local, o valor padrão já funciona. Se for usar o MongoDB Atlas, edite a variável `DB_CONNECTION_STRING` no novo arquivo `.env`.
...
A maneira mais simples de rodar a aplicação integrada ao banco de dados é utilizando o Docker Compose, que já configura a rede entre a API e o MongoDB:
...
O fluxo de trabalho (`.github/workflows/main.yml`) é acionado em todo _push_ ou _Pull Request_ para a branch `main`.
Existe cache de dependências no ambiente do runner, build otimizada com Dockerfile multi-stage e versionamento semântico no push para Docker Hub.
